...
describe("MemoryPage", () => {
  ...
  it("shows empty when no memories", () => {
    vi.mocked(useMemoryStore).mockImplementation(() => createMockStore({ memories: [] }) as never);
    render(
      <BrowserRouter>
        <MemoryPage />
      </BrowserRouter>
  ...
  it("shows skeletons when loading + empty + memories tab", () => {
    vi.mocked(useMemoryStore).mockImplementation(() => createMockStore({ memories: [], loading: true }) as never);
    render(
      <BrowserRouter>
        <MemoryPage />
      </BrowserRouter>
  ...
  it("shows conflict badge with count when conflicts exist", () => {
    ...
    vi.mocked(useMemoryStore).mockImplementation(() => createMockStore({ conflicts }) as never);
    render(
      <BrowserRouter>
        <MemoryPage />
      </BrowserRouter>
  ...
  it("renders 已合并 tag when memory has superseded_by", () => {
    const memWithSuper = baseMemory({
      ...
      content: "Old L1",
      level: "L1",
      superseded_by: "abcd1234efgh5678",
    });
    vi.mocked(useMemoryStore).mockImplementation(() => createMockStore({ memories: [memWithSuper] }) as never);
    render(
      <BrowserRouter>
        <MemoryPage />
      </BrowserRouter>
  ...
  it("renders conflict tag when memory is in conflict group and is_current=0", () => {
    ...
    vi.mocked(useMemoryStore).mockImplementation(() => createMockStore({ memories: [memSuperseded] }) as never);
    render(
      <BrowserRouter>
        <MemoryPage />
      </BrowserRouter>
