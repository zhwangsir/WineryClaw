...
export const configApi = {
  ...
  createWorkspace: (data: Partial<Workspace>) =>
    api.post<{ workspace: Workspace }>("/api/config/workspace", data).then((r) => r.workspace),
  workspaceAgents: (id: string) =>
    api.get<{ agents: AgentConfig[] }>(`/api/config/workspace/${id}/agents`).then((r) => r.agents),
  agent: (wid: string, aid: string) =>
    api.get<{ agent: AgentConfig }>(`/api/config/agent/${wid}/${aid}`).then((r) => r.agent),
